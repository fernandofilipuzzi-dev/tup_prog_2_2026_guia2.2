...
namespace Ejercicio5.Models
{
    class ControlRadar
    {
        ...
        public Vehiculo BuscarPorPatente(string patente)
        {
            ...
            {
                c = (n + m) / 2;
                if (infracciones[c].Patente.ToUpper().CompareTo(patente) == 0)
                {
                    buscado = infracciones[c];
                }
                else if (infracciones[c].Patente.ToUpper().CompareTo( patente )<0)///esta a la derecha
                {
                    n = c + 1; 
                }
                else if (infracciones[c].Patente.ToUpper().CompareTo(patente) > 0)///esta a la izquierda
                {
                    m = c - 1;
                }
            }
